	...
	if (qa_clicked('doask')) {
		...
		try {
			...
			if (strlen($NoNIK_input) && strlen($NamaNIK_input) && strlen($EmailUser_input)) {
				    $errorVerifikasi='NIK atau Nama Lengkap tidak valid. Mohon periksa kembali, NIK dan Nama Lengkap harus sesuai dengan KTP Anda yang terdaftar pada http://data.kpu.go.id/dps2015.php';
					$curl = curl_init();
                     curl_setopt_array($curl, array(
                     CURLOPT_RETURNTRANSFER => 1,
					 /*CURLOPT_URL => "https://data.kpu.go.id/search.php?cmd=cari&nik=".$NoNIK_input,*/
                     CURLOPT_URL => "https://www.kawalpilkada.id/cekktp/json/".$NoNIK_input,
                     ));
